...
cdef class smcTranslocatorDirectional(object):
    ...
    cdef cython.double [:] coDirCollisionFalloff
    ...
    cdef cython.long [:] collision1 # list of whether SMC has collided with an RNAP (by default only flags collisions where RNAP runs into one of the legs); once flag is flipped it stays on until unbinding    
    cdef cython.long [:] collision2
    cdef cython.long [:] coDirCollision1 # flags co-directional collisions; simply detects if RNAP has a cohesin trailing it in site immediately adjacent; flag is reset every time step
    cdef cython.long [:] coDirCollision2
    cdef double lefperm
    ...
    def __init__(self, emissionProb, deathProb, stallProbLeft, stallProbRight, unstallProb, pauseProb, stallFalloffProb, numSmc, 
                 ...
                 perm_occ=[], 
                 collisionFalloffProb=[], coDirCollisionFalloffProb=[],
                 poldissoc=0.,
                 LefPerm=0.,
                 strongCTCF=0
                ):
        ...
        self.coDirCollision1 = np.zeros(self.M, int) - 1
        self.coDirCollision2 = np.zeros(self.M, int) - 1
        ...
        if type(coDirCollisionFalloffProb) in [int,float,np.float64,np.double]:
            self.coDirCollisionFalloff=np.ones(self.N, np.double)*coDirCollisionFalloffProb
        elif len(coDirCollisionFalloffProb)==0:
            self.coDirCollisionFalloff=np.array([deathProb]*self.N)
        else:
            self.coDirCollisionFalloff=coDirCollisionFalloffProb
        ...
        if type(kinpol) in [int,float,np.float64,np.double]: # not the preferred method of initialization since it will make kter>0 including in genes
            self.kin=np.ones(self.N, np.double)*kinpol
        else:
                        ...
                        if (self.PolPos[i+m+j]==0) and ((i+m+ncoright+ncoleft)<self.N-2):  # Check if there is a Pol II right after the cohesins, if not, then the trailing pol will push all the nco cohesins forward.   
                            if randnum()<self.PausePol[i+m]:
                                ...
                                if not stalled_lef: #stalled_lef=False when strongStall=False
                                    ...
                                    #so how will we deal with co-directional collisions?
                                    #two ways:
                                    #1) compatible with the above, add an indicator for when cohesin would attempt to advance to a site, but cannot because RNAP is there. then flip bool for indicated LEFs
                                    #2) instead, change scheme to one where we cycle through all RNAPs and check if there's an SMC in front of or behind it, then change its bool
                                    #3) one more possibility is have each cohesin check if there's an RNAP in front of it.
                                m=m+1
                    if self.PolPos[itt+self.PolLoadingSite[it]]==1:
                        #check if there's a co-directional cohesin at itt+self.PolLoadingSite[it]-1
                        if self.occupied[itt+self.PolLoadingSite[it]-1]==1:
                            for k in range(self.M):
                                if self.SMCs2[k]==itt+self.PolLoadingSite[it]-1:
                                    self.coDirCollision2[k]=itt+self.PolLoadingSite[it]
                                    break 
                    ...
                    # Make sure the termination set is not at the beginning of the lattice
                        ...
                        if (self.PolPos[i-m-j]==0) and ((i-m-ncoright-ncoleft)>1):  # Check if there is a Pol II right after the cohesins, if not, then the trailing pol will push all the nco cohesins forward.   
                            if randnum()<self.PausePol[i-m]:
                                ...
                                m=m+1
                    if self.PolPos[self.PolLoadingSite[it]-itt]==1:
                        #check if there's a co-directional cohesin at itt+self.PolLoadingSite[it]-1
                        if self.occupied[self.PolLoadingSite[it]-itt+1]==1:
                            for k in range(self.M):
                                if self.SMCs1[k]==self.PolLoadingSite[it]-itt+1:
                                    self.coDirCollision1[k]=self.PolLoadingSite[it]-itt 
                                    break
        return
    ...
    cdef death(self):
        ...
        for i in range(self.M):
            falloff = max(self.falloff[self.SMCs1[i]], self.falloff[self.SMCs2[i]])
            #stall falloff rate dominates bare falloff
            if self.stalled1[i] == 1:
                if self.stalled2[i] == 1:
                    falloff=max(self.stallFalloff[self.SMCs1[i]], self.stallFalloff[self.SMCs2[i]])
                else:
                    falloff= self.stallFalloff[self.SMCs1[i]]
            elif self.stalled2[i] == 1:
                falloff= self.stallFalloff[self.SMCs2[i]]
            
            #co-directional collisions can add stability over stall falloff or bare falloff
            if self.coDirCollision1[i] > -1:
                if self.coDirCollision2[i] > -1:
                    #co-dir collisions tend to conserve cohesin, so take min deathrate
                    falloff=min(self.coDirCollisionFalloff[self.SMCs1[i]], self.coDirCollisionFalloff[self.SMCs2[i]])
                else:
                    falloff=self.coDirCollisionFalloff[self.SMCs1[i]]
            elif self.coDirCollision2[i] > -1:
                falloff=self.coDirCollisionFalloff[self.SMCs2[i]] 

            #head-on collisions dominate what happens to fall off rate
            if self.collision1[i] > -1:
                if self.collision2[i] > -1:
                    falloff= max(self.collisionFalloff[self.SMCs1[i]], self.collisionFalloff[self.SMCs2[i]])
                else:
                    falloff = self.collisionFalloff[self.SMCs1[i]]
            elif self.collision2[i] > -1:
                falloff = self.collisionFalloff[self.SMCs2[i]]            
            ...
            if randnum() < falloff:                 
                ...
                self.collision2[i] = -1
                self.birth(i)
            #these two always get reset and checked again next timestep
            self.coDirCollision1[i]= -1
            self.coDirCollision2[i]= -1
